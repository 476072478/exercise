          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="item in spuSaleAttrList" :key="item.id">
                ...
                <dd
                  changepirce="0"
                  :class="c1.isChecked == 1 ? 'active' : ''"
                  v-for="c1 in item.spuSaleAttrValueList"
                  :key="c1.id"
                  @click="handlename(c1, item.spuSaleAttrValueList)"
                >
                  {{ c1.saleAttrValueName }}
                </dd>
              </dl>
            </div>
            <div class="cartWrap">
...
export default {
  ...
  methods: {
    handlename(a, b) {
      b.forEach((item) => {
        item.isChecked = "0";
        if (a.saleAttrValueName == item.saleAttrValueName) {
          item.isChecked = "1";
        }
      });
    },
        ...
        .then((res) => {
          if (res.status == 200) {
            sessionStorage.setItem(
              "list",
              JSON.stringify(Object.assign(this.skuInfo, this.spuSaleAttrList))
            );
            this.$router.push({
              path: "/cart/" + this.skuNum,
            });
          } else {
            alert("失败");
          }
