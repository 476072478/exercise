				...
				<view class="feed-one" v-for="item in screenInfo" :key="item.id">
					<navigator open-type="navigate" :url="'/subpages/feedinfo/feedinfo?id=' + item.id">
						...
						<view class="feed-info">
							...
							<view class="iview">
								<view class="ilike">
									<image src="@/static/viewRouter.png" class="micon" />
									<text class="love-count" v-if="item.viewNum > 0">{{ item.viewNum }}</text>
								</view>
							</view>
						</view>
					</navigator>
...
<script>
import { requestmyfoot } from '@/common/http.api.js';
...
export default {
	...
	onLoad(params) {
		//获取瀑布流数据
		let user_id = JSON.parse(uni.getStorageSync('userInfo')) && JSON.parse(uni.getStorageSync('userInfo')).id;
		requestmyfoot(user_id).then(res => this.getScreenInfo(res.data));
	},
	computed: {
		...mapState(['Introduce'])
	},
	methods: {
		...
		getScreenInfo(data) {
			let arr = this.Introduce.filter(item => data.includes(JSON.stringify(item.id))
			);
			this.screenInfo = arr;
		}
	}
