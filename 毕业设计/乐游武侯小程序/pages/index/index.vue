		...
		<view class="header-box">
			...
			<view class="card-header">
				<view class="card-one card-left" @tap="gotoSearch('/subpages/search/search')">
					...
					<view class="iright"><view class="title">查询景区</view></view>
				</view>
				<view class="card-one card-right" @tap="gotoSearch('/subpages/recommend/recommend')">
					...
					<view class="iright"><view class="title">线路推荐</view></view>
				</view>
			</view>
								...
								<view class="feed-info">
									...
									<view class="iview">
										<view class="ilike" @tap.stop="checkLove(item)">
											<image v-if="usercheck.includes(item.id)" src="@/static/ysc.png" class="micon" />
											<image v-else src="@/static/sc.png" class="micon" />
				...
				<view :style="{ height: newsHeight }">
					<view v-for="(item, index) in Introduce" :key="index">
						<navigator class="one-new" open-type="navigate" :url="'/subpages/newinfo/newinfo?id=' + item.id">
							...
							<view class="right"><image class="pic" mode="aspectFill" :src="item.imgsrc" /></view>
						</navigator>
					</view>
				</view>
			</swiper-item>
...
export default {
	...
	onLoad() {
		...
		//获取动态瀑布流信息
		this.getIntroduceInfo().then(() => {
			this.myref(), (this.usercheck = this.getUser_check());
		});
	},
	onShow() {
		//获取用户点赞信息
	...
	methods: {
				...
				setTimeout(() => {
					wx.createSelectorQuery()
						...
						.boundingClientRect(rect => {
							console.log(rect.height / 2.4 + 'px');
							this.swpierHight = rect.height / 2.4 + 'px';
							this.oldHeight = rect.height / 2.4 + 'px';
						})
						.exec();
				}, 1000);
			}
		...
		monitorswiper(index) {
			...
			if (index.detail.current === 1) {
				//调整讨论页面高度
				this.newsHeight = this.Introduce && this.Introduce.length * 100 + 100 + 'px';
